...
namespace std {
  ...
  template<typename R>
  concept process_argument_list =
     (
        convertible_to<iter_value_t<ranges::iterator_t<R>>, string_view> ||
        convertible_to<iter_value_t<ranges::iterator_t<R>>, wstring_view> ||
        convertible_to<iter_value_t<ranges::iterator_t<R>>, u8string_view>
    );
  ...
  template<typename T, typename Args = vector<string>>
  concept process_launcher = 
    process_argument_list<Args> &&
    requires(T launcher, Args a, filesystem::path p) {
            // Takes an error_code, so initializers can report internal errors
            { launcher.set_error(error_code(), "message") } -> std::same_as<void>;
            { launcher.launch(p, a) } -> std::same_as<process>;
    };
  ...
  template<typename Launcher = default_process_launcher, typename Init = default_process_launcher>
  concept process_initializer =
